...
fun main(){
    val rawApiData: List<Map<String, Any?>> = listOf(
        mapOf("id" to "E01", "name" to "Laptop", "type" to "ELECTRONIC", "warranty_months" to 24),
        mapOf("id" to "C01", "name" to "T-Shirt", "type" to "CLOTHING", "size" to "XL"),
        mapOf("id" to "E02", "name" to "Mouse", "type" to "ELECTRONIC", "warranty_months" to "Not An Integer"), // Corrupted warranty
    ...
    for(raw in rawApiData){
        try{
            val result: Product? = parser.parseProduct(raw)
            result?.let {
